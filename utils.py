...
def plot_singular_values(sv,LM_TRUNC,ax=None,title="Affine mapping regularization"):
    ...
    #ax.set_yscale("log")
    plt.xticks(jj)
    plt.axvline(LM_TRUNC+.5,linestyle='dashed',c='r')
    plt.grid(True)
    plt.ylabel('$j^{th}$ singular value, $\Lambda_{jj}\,/\,\Lambda_{11}$')
    plt.xlabel('$j$')
    plt.text(-.03,1.05,title,transform=ax.transAxes,fontsize=20)